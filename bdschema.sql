...
CREATE TABLE IF NOT EXISTS bdschema.Vehicule (
    ...
    killometrage            INTEGER                             -- killometraqge au compteur
    immatriculation         VARCHAR(6)
    consommation            INTEGER                 NOT NULL

    PRIMARY KEY (vid)
    FOREIGN KEY (localisation) REFERENCES bdschema.Emplacement(eid) ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS bdschema.Reservation (
    ...
    dateDebut               DATETIME                NOT NULL    -- date de debut de la reservation
    dateFin                 DATETIME                NOT NULL    -- date fin de la reservation
    specifications          TEXT
    ...
    FOREIGN KEY (vehicule, killometrageDebut, killometrageFin) REFERENCES bdschema.Vehicule (vid, killometrage, killometrage)
